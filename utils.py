...
def make_optimizer(args, my_model, pretrain=False):
    ...
    if pretrain is False:
    ...
    elif pretrain is True:
        if args.optimizer_pretrain == 'SGD':
            optimizer_function = optim.SGD
            kwargs = {'momentum': 0.9,
                    'lr': args.lr_pretrain,
                    'weight_decay': 0#args.weight_decay
            }
        elif args.optimizer_pretrain == 'Adam':
            optimizer_function = optim.Adam
            kwargs = {
                'betas': (0.9, 0.999),
                'eps': 1e-08,
        ...
        elif args.optimizer_pretrain == 'LBFGS':
            optimizer_function = optim.LBFGS
            kwargs = {'lr': args.lr_pretrain,
                    'history_size': args.history_size,
                    'line_search_fn': 'strong_wolfe'
            }
...
def make_scheduler(args, my_optimizer, pretrain=False):
    if pretrain is False:
        ...
        if args.decay_type == 'step':
            scheduler = lrs.StepLR(
                my_optimizer,
                step_size=args.patience_pretrain,
                gamma=args.gamma_pretrain
            )
        elif args.decay_type.find('step') >= 0:
            milestones = args.decay_type.split('_')
            milestones.pop(0)
